...
pub fn render(frame: &mut Frame, area: Rect, app: &App) {
    ...
    // Income sparkline
    let income_data: Vec<u64> = app.state.income_history.iter()
        .map(|v| (v * 100.0).max(0.0) as u64)
        .collect();
    let income_spark = Sparkline::default()
        .block(Block::default()
            .title(Span::styled(" Income ", Style::default().fg(theme::ACCENT_GREEN)))
            .borders(Borders::ALL)
    ...
    // Expense sparkline
    let expense_data: Vec<u64> = app.state.expense_history.iter()
        .map(|v| (v * 100.0).max(0.0) as u64)
        .collect();
    let expense_spark = Sparkline::default()
        .block(Block::default()
            .title(Span::styled(" Expenses ", Style::default().fg(theme::ACCENT_RED)))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(theme::BORDER))
            .style(Style::default().bg(theme::BG)))
        .data(&expense_data)
    ...
    let net_data: Vec<u64> = app.state.income_history.iter()
        .zip(app.state.expense_history.iter())
        .map(|(i, e)| ((i - e) * 100.0).max(0.0) as u64)
        .collect();
    let net_spark = Sparkline::default()
        .block(Block::default()
            .title(Span::styled(" Net Profit ", Style::default().fg(theme::ACCENT_CYAN)))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(theme::BORDER))
            .style(Style::default().bg(theme::BG)))
        .data(&net_data)
