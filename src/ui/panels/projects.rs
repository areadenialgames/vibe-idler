...
pub fn render(frame: &mut Frame, area: Rect, app: &App) {
    let block = Block::default()
        .title(Span::styled(" Active Projects ", Style::default().fg(theme::ACCENT_YELLOW).bold()))
        .borders(Borders::ALL)
    ...
    let active_count = app.state.active_projects.len().min(5);
    let active_height = (active_count as u16 * 2).max(2);
    let remaining = inner.height.saturating_sub(active_height + 2);
    ...
    for proj in app.state.active_projects.iter().take(5) {
    ...
    if lines.is_empty() {
        lines.push(Line::from(Span::styled("  No active projects", Style::default().fg(theme::DIM))));
    }
    ...
    if !app.state.passive_income_sources.is_empty() {
        let total_passive: f64 = app.state.passive_income_sources.iter().map(|p| p.monthly_income).sum();
        bottom_lines.push(Line::from(""));
        bottom_lines.push(Line::from(vec![
            Span::styled(
                format!(" Passive: {}/mo ({} sources)",
